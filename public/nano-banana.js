// Nano Banana AI - Production Version with All Features
    ...
    try {
        for (let i = 0; i < 4; i++) {
            ...
            if (imageElement && imageElement.src) {
                ...
                actions.innerHTML = `
                    <button class="nb-btn-secondary" style="flex: 1; padding: 0.5rem; font-size: 0.85rem;" onclick="saveVariant('${imageData}', '${fullPrompt.replace(/'/g, "\\'")}',' ${selectedModel}', '${styleKey}')">♥️ 保存</button>
                    <a href="${imageData}" download="banana-${i + 1}.png" class="nb-btn-secondary" style="flex: 1; padding: 0.5rem; font-size: 0.85rem; text-align: center;">⬇️ 下載</a>
                `;
...
async function generateImage() {
    ...
    try {
        ...
        if (!imageElement || !imageElement.src) throw new Error('生成失敗');
        ...
        resultContainer.innerHTML = `
            <div style="text-align: center;">
                ...
                <p style="color: var(--nb-text-secondary); font-size: 0.9rem; margin-bottom: 1rem;">模型: ${modelName} | 風格: ${styleDescriptions[styleKey] || '無'} | 解析度: ${resolution}</p>
            </div>
        `;
        ...
        actionsDiv.innerHTML = `
            <a href="${imageData}" download="banana-${Date.now()}.png" class="nb-btn-primary" style="flex: 1;">
        ...
        clearInterval(progressInterval);
        resultContainer.innerHTML = `<div style="text-align: center; padding: 2rem;"><p style="color: var(--nb-error); font-weight: 600;">❌ 生成失敗: ${error.message}</p></div>`;
